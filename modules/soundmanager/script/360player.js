...
function ThreeSixtyPlayer() {
  ...
  this.config = {
	...
	hide360: false, // show/hide the 360 player
  ...
  this.events = {
    ...
    finish: function() {
      ...
      if (pl.config.playNext) {
		...
		{
		if (sm.radioclashSpecialFeatures && sm.radioclashSpecialRedirectUrl != '')
			window.location.href = sm.radioclashSpecialRedirectUrl;
		}
      }
    },
...
//Ajout Gaëtan (DEBUT) : On gère le click sur les raccourcis (class="myShortcut")
	  var classAttributeName = 'class';
	  if (o.getAttribute('class') == null)
	      classAttributeName = 'className';
	  
	  if (self.config.divagSpecialFeatures) {
		if (o.getAttribute(classAttributeName).indexOf('myShortcut') == 0)
		{
...
//Ajout Gaëtan (DEBUT) : On affiche uniquement le lecteur en cours.
    if (self.config.divagSpecialFeatures) {
	  ...
	  var elementsToHide = self.getElementsByClassName('sm2-inline-list','div');
	  for (var i=0; i<elementsToHide.length; i++)
	      elementsToHide[i].style.display = 'none';
		  
	  // - On afficher que celui qui va jouer
	  if (!self.config.hide360)
		o.parentNode.parentNode.style.display = 'block';	 

	  // - On masque remet tous les raccourcis en class="myShortcut"
	  var elementsToStyle = self.getElementsByClassName('myShortcut','a');
	  for (var i=0; i<elementsToStyle.length; i++)
	  {
 	    if (elementsToStyle[i].getAttribute('cible') != o.id)
		{
	      elementsToStyle[i].setAttribute(classAttributeName, "myShortcut");
		}
		else
		{
     	  // - On met tous les raccourcis du morceau en cours avec le style approprié :
		  if (elementsToStyle[i].getAttribute(classAttributeName) == "myShortcut myShortcutPlaying")
		  {
		    //soit en pause (class="myShortcut myShortcutPaused")
			elementsToStyle[i].setAttribute(classAttributeName, "myShortcut myShortcutPaused");
		  }
		  else
		  {
  		    //soit en play (class="myShortcut myShortcutPlaying")
			elementsToStyle[i].setAttribute(classAttributeName, "myShortcut myShortcutPlaying");
		  }
		}
	  }
	}
...
this.updatePlaying = function() {
  ...
  this.init = function() {
    ...
    if (foundItems>0) {
	  ...
	  {
		if (soundManager.radioclashSpecialFeatures && soundManager.radioclashSpecialFirstPlay != -1)
		{
			var first = soundManager.radioclashSpecialFirstPlay;
			self.handleClick({target:self.links[first],preventDefault:function(){}});
			self.handleClick({target:self.links[first],preventDefault:function(){}});
		}
	  }
    }
...
ThreeSixtyPlayer.prototype.Metadata = function(oSound, oParent) {
  ...
  this.refresh = function() {
    ...
    if (index != metadata.currentItem && index < metadata.length) {
      // update
      oSound._360data.oLink.innerHTML = metadata.mainTitle+' <span class="metadata"><span class="sm2_divider"> ' + soundManager.metadataSeparator + ' </span><span class="sm2_metadata">'+metadata[index].title+'</span></span>';
